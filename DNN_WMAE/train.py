...
def WMAE(opt, tar):
    return torch.mean(torch.sum(torch.mm(torch.abs(opt - tar), torch.Tensor([[300], [1], [200]]).cuda()), dim = 1))
...
def main():
    ...
    try:
        ...
        for epoch in range(1, epochs + 1):
            ...
            print('Epoch: {0:3d}/{1:3d} (Elapsed {4:2.3f}s)\n ~~> TrnLoss: {2[0]:3.5f} / {2[1]:3.5f} / {2[2]:3.5f}\t\tVldLoss: {3[0]:3.5f} / {3[1]:3.5f} / {3[2]:3.5f}'.format(epoch, epochs, trainScore, validScore, time.time() - beginTS))

            if epoch % 20 == 0:
                torch.save(model, 'models/model_{:03d}.pkl'.format(epoch))
