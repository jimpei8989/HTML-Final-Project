...
def score(predict, truth):
    return (np.mean(np.sum(np.abs(predict - truth), axis = 1)),
...
def HuberLoss(pred, truth):
    d = pred - truth
    scale = 1 + d ** 2
    sqrtScale = np.sqrt(scale)
    return d / sqrtScale, 1 / scale / sqrtScale
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i, name in enumerate(targets):
            model[i] = XGBRegressor(boosting_type = 'gbtree',
